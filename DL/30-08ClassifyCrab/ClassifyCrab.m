%% Crab Classification
% This example illustrates using a neural network as a classifier to
% identify the sex of crabs from physical dimensions of the crab.

%% The Problem: Classification of Crabs
% In this example we attempt to build a classifier that can identify the
% sex of a crab from its physical measurements. Six physical
% characteristics of a crab are considered: species, frontallip, rearwidth,
% length, width and depth. The problem on hand is to identify the sex of a
% crab given the observed values for each of these 6 physical
% characteristics.
%
%% Why Neural Networks?
...
% neural networks are certainly a good candidate for solving the problem.
%
% The six physical characteristics will act as inputs to a neural network
% and the sex of the crab will be the target. Given an input, which
% constitutes the six observed values for the physical characteristics of a
% crab, the neural network is expected to identify if the crab is male or
% female.
%
...
% matrix T.
%
% Each ith column of the input matrix will have six elements representing a
% crab's species, frontallip, rearwidth, length, width, and depth.
%
% Each corresponding column of the target matrix will have two elements.
% Female crabs are represented with a one in the first element, male crabs
...
%
% Performance is measured in terms of mean squared error, and is shown in a
...
% This will give us a sense of how well the network will do when applied to
