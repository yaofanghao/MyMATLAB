...
% cluster iris flowers into classes topologically, providing insight into
% the types of flowers and a useful tool for further analysis.
...
% for the known inputs, but will also let us classify unknown inputs
% accordingly.
%
...
%
% The four flower attributes will act as inputs to the SOM, which will map
% them onto a 2-dimensional layer of neurons.
%
%% Preparing the Data
% Data for clustering problems are set up for a SOM by organizing the data
% into an input matrix X.
%
% Each ith column of the input matrix will have four elements representing
% the four measurements taken on a single flower.
%
...
%
% *selforgmap* creates self-organizing maps for classifying samples with as
% much detail as desired by selecting the number of neurons in each
...
% with more neurons, and more dimensions allows for modelling the
...
% training and the criteria which stopped training will be highlighted in
% green.
%
...
% populated by the known flowers, and can now be used to classify new
% flowers accordingly.  The network output will be a 64x150 matrix, where
% each ith column represents the jth cluster for each ith input vector with
% a 1 in its jth element.
%
% The function *vec2ind* returns the index of the neuron with an output of
% 1, for each vector.  The indices will range between 1 and 64 for the 64
% clusters represented by the 64 neurons.
...
% represent a different class of flower, with adjacent neurons typically
...
% the 64 neurons in the 8x8 hexagonal grid.  Darker colors represent larger
% weights.  If two inputs have similar weight planes (their color gradients
% may be the same or in reverse) it indicates they are highly correlated.
